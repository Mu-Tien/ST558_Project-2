...
```{r}
# create a random forest model using cross-validation
...
# create a random forest model using cross-validation
random_forest <- train(shares ~ .,
                    data=news_train,
                    method='rf',
                    metric='MAE',
                    tuneGrid = expand.grid(mtry=seq(0,1.5,0.05)),
...
```{r}
# compare model to the test set
rf_pred <- predict(random_forest, newdata=news_test)
postResample(rf_pred, news_test$shares)
```
